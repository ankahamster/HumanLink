...
生物认证授权技术路线的分类应基于技术问题而非厂商：**可信边界在哪、谁负责编排、私钥在哪、输出什么、授权对象是什么、开放性如何**。按此维度分为四类：

| 维度         | 分立式认证器 + SE（HumanLink参考）                     | 一体化安全域架构                        | 平台编排型 TEE / StrongBox                        | 硬件认证器 / 设备认证                 |
| ---------- | -------------------------------------------- | ------------------------------- | -------------------------------------------- | ---------------------------- |
| 典型形态       | 生物模块 + MCU + SE + SDK                        | 传感器 + SoC + Secure Enclave + OS | BiometricPrompt + TEE/Keystore/StrongBox/eSE | YubiKey、FIDO2 key、PIV card   |
| 安全边界       | 分散，需 Secure Boot + 链路保护 + SE 签名补强            | 最集中，模板/匹配/密钥在同一安全域              | 中等到高，取决于 OEM 实现                              | 独立认证器边界，主机不持有私钥              |
| 操作绑定能力     | **强**：绑定 actionHash + Challenge + 设备 DID     | 中等：支付确认强，但不开放为通用操作断言            | 弱到中等：主要绑定系统认证请求                              | 中等：可绑定 challenge，默认语义为登录认证   |
| 第三方验证与举证   | **强**：独立验签 + 查设备状态 + 撤销记录 + 生成审计证据           | 弱：依赖本机或生态 API                   | 弱到中等：依赖系统 API 和 OEM 实现                       | 中到强：RP 可验证签名，但不表达具体人类授权      |
| 审计/状态层     | Verifier + Registry + Revocation + Audit log | 主要为系统内部日志                       | 主要为系统内部安全日志                                  | RP 认证记录、设备证书状态               |
| 开放性        | **高**：可替换模组，接入不同平台                           | 低：强绑定硬件生态                       | 中等：生态内兼容强但 OEM 差异大                           | 高：FIDO/WebAuthn 标准成熟，跨平台部署广  |
| Agent 场景适配 | **强**：拦截高风险 Agent 行为，生成授权证明                  | 弱：非为 Agent 授权设计                 | 弱到中等：可辅助登录，难绑定具体 action                      | 中等：可做强认证，不天然表达 actionHash 授权 |
