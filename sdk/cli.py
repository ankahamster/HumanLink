...
AUDIT_PROMPT_ENABLED = False
AUDIT_UI_SDK_URL = "http://127.0.0.1:8765"
...
def _on_auth_result(approval_id: str, decision: str, assertion: dict | None = None, command: str = ""):
    ...
    if success and assertion:
        _prompt_audit_record_choice(assertion, action, source="demo")


def _create_humanlink_record_url(assertion: dict, action: str = "", source: str = "cli") -> str | None:
    """Create an audit session via the running SDK API and return the record URL."""
    try:
        import requests
        ...
        payload = {
            ...
            "action": action,
            "source": source,
            "sdk_url": AUDIT_UI_SDK_URL,
        }
        response = requests.post(
            f"{AUDIT_UI_SDK_URL}/audit/session",
            json=payload,
            timeout=20,
        )
        if not response.ok:
            raise ValueError(f"SDK API returned {response.status_code}: {response.text}")

        audit_session_id = response.json().get("audit_session_id", "")
        if not audit_session_id:
            raise ValueError("No audit_session_id returned from SDK API")
        return f"{AUDIT_UI_SDK_URL}/ui/humanlink_record?{urlencode({'audit_session_id': audit_session_id})}"
    except Exception as error:
        console.print(f"[yellow]Failed to prepare HumanLink record UI:[/] {error}")
        return None


def _prompt_audit_record_choice(assertion: dict, action: str = "", source: str = "cli"):
    """Open the browser UI for audit handling."""
    if not AUDIT_PROMPT_ENABLED or not assertion:
        return

    ui_url = _create_humanlink_record_url(assertion, action, source=source)
    if not ui_url:
        return

    console.print(f"[cyan]Opening HumanLink record UI:[/] [link={ui_url}]{ui_url}[/link]")
    opened = bool(webbrowser.open(ui_url))
    if opened:
        console.print("[dim]Use the HumanLink record page to decide whether to keep the record local or anchor it on-chain.[/]")
        console.print()
        return

    console.print("[yellow]Browser did not open automatically. Open the URL above manually to continue in the HumanLink record page.[/]")
    console.print()
...
def handle_demo(args):
    ...
    global AUDIT_PROMPT_ENABLED, AUDIT_UI_SDK_URL
    AUDIT_PROMPT_ENABLED = True
    from bridge.tui import render_auth_request, render_waiting_fingerprint, \
        render_verification_progress, render_result
    from bridge.notifier import notify
    ...
    while time.time() < deadline:
        try:
            ...
            if status == "completed":
                ...
                _prompt_audit_record_choice(
                    assertion,
                    assertion.get("challenge", {}).get("action", command),
                    source="cli",
                )
                return
