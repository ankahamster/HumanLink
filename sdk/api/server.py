...
from chain_audit import anchor_assertion_summary
...
HUMANLINK_ROOT = SDK_ROOT.parent
CONTRACTS_DIR = HUMANLINK_ROOT / "contracts"
UI_DIR = SDK_ROOT / "ui"
...
class AuditSessionCreateRequest(BaseModel):
    """Create an audit session used by the local browser UI."""
    assertion: Dict[str, Any]
    action: str = ""
    source: str = "cli"
    sdk_url: str = "http://127.0.0.1:8765"
    auth_session_id: str = ""


class AuditSessionResponse(BaseModel):
    """Audit session payload returned to the local UI."""
...
def _read_env_file(env_path: Path) -> Dict[str, str]:
    """Read a simple .env file into a dictionary."""
    values: Dict[str, str] = {}
...
def _load_contract_settings() -> Dict[str, str]:
    """Load contract-related settings from environment and contracts/.env."""
    env_values = _read_env_file(CONTRACTS_DIR / ".env")
    merged = {**env_values, **{key: value for key, value in os.environ.items() if value is not None}}
    return merged


def _resolve_assertion_registry_address() -> Optional[str]:
    """Resolve the deployed AssertionStatusRegistry address."""
    settings = _load_contract_settings()
    address = settings.get("ASSERTION_STATUS_REGISTRY_ADDRESS")
    if address and address != "0x0000000000000000000000000000000000000000":
        return address

    deployment_path = CONTRACTS_DIR / "deployments" / "sepolia.json"
    if deployment_path.exists():
        deployment = json.loads(deployment_path.read_text(encoding="utf-8"))
        return deployment.get("contracts", {}).get("AssertionStatusRegistry")

    return None


def _build_audit_config() -> Dict[str, Any]:
    """Return frontend-friendly audit configuration."""
    settings = _load_contract_settings()
    return {
        "network": settings.get("HUMANLINK_WALLET_NETWORK", "sepolia"),
        "chainId": int(settings.get("HUMANLINK_WALLET_CHAIN_ID", "11155111")),
        "rpcUrl": settings.get("SEPOLIA_RPC_URL", ""),
        "contractAddress": _resolve_assertion_registry_address(),
        "walletModeAvailable": bool(_resolve_assertion_registry_address()),
    }
...
@app.get("/ui/openclaw-approval")
async def get_openclaw_approval_ui():
    """Serve the HumanLink approval page used by the OpenClaw bridge."""
    ui_path = UI_DIR / "openclaw_approval.html"
    if not ui_path.exists():
        raise HTTPException(status_code=404, detail="Approval UI not found")

    return FileResponse(ui_path)


def _create_audit_session_payload(
    *,
    assertion: Dict[str, Any],
    action: str = "",
    source: str = "cli",
    ...
    """Create and store an audit session for the browser UI."""
    import uuid
...
@app.get("/ui/humanlink_record")
@app.get("/ui/audit-record-choice")
async def get_humanlink_record_ui():
    """Serve the HumanLink record page used by cli.py success flows."""
    ui_path = UI_DIR / "humanlink_record.html"
    if not ui_path.exists():
        raise HTTPException(status_code=404, detail="HumanLink record UI not found")
...
async def get_audit_config():
    """Return on-chain audit configuration for local UI clients."""
    return _build_audit_config()


@app.post("/audit/session", response_model=AuditSessionResponse)
async def create_audit_session(request: AuditSessionCreateRequest):
    """Create a browser-friendly audit session from a verified assertion."""
    if not request.assertion:
        raise HTTPException(status_code=400, detail="Missing assertion payload")
    return _create_audit_session_payload(
        assertion=request.assertion,
        action=request.action,
        source=request.source,
        sdk_url=request.sdk_url,
        auth_session_id=request.auth_session_id,
    )


@app.get("/audit/session/{audit_session_id}", response_model=AuditSessionResponse)
async def get_audit_session(audit_session_id: str):
    """Fetch an audit session payload for the local browser UI."""
    if audit_session_id not in audit_sessions:
...
@app.post("/audit/anchor/{session_id}")
async def anchor_audit_record(session_id: str):
    """
    Anchor the completed session's assertion with the server-side wallet.

    This keeps the current Hardhat-based backend signing mode available even
    after the UI-based wallet mode is introduced.
    """
    session = sessions.get(session_id)
    audit_session = audit_sessions.get(session_id)
    assertion = None

    if session:
        if session.get("status") != "completed":
            raise HTTPException(status_code=400, detail="Only completed authentication sessions can be anchored")

        verification_result = session.get("verification_result") or {}
        if verification_result.get("valid") is not True:
            raise HTTPException(status_code=400, detail="Only valid verification results can be anchored")

        assertion = session.get("assertion")
    elif audit_session:
        assertion = audit_session.get("assertion")
    else:
        raise HTTPException(status_code=404, detail="Session not found")

    if not assertion:
        raise HTTPException(status_code=400, detail="No completed assertion is available for this session")

    success, message = anchor_assertion_summary(assertion, contracts_dir=CONTRACTS_DIR)
    if not success:
