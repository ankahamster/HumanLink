...
class USBBridge:
    ...
    def _send_command(self, command: Dict[str, Any], timeout: Optional[float] = None) -> Dict[str, Any]:
        ...
        # Read response - ESP32 may emit startup logs or event notifications
        # (for example {"event":"ready", ...}) before the actual command reply.
        # Keep those notifications available in logs, but continue waiting until
        # we receive a response frame for this command.
        ...
        while time.time() - start_time < timeout:
            try:
                if self.connection.in_waiting > 0:
                    ...
                    if line:
                        # Try to parse as JSON. Event-style frames are valid
                        # notifications from firmware, but they are not the
                        # response to the command we just sent.
                        try:
                            response = json.loads(line)
                            if "event" in response and "status" not in response:
                                logger.info(f"Device event while waiting for response: {response}")
                                continue
                            return response
            ...
            raise TimeoutError(f"No command response JSON received within {timeout} seconds")
