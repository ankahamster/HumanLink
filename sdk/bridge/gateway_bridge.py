...
class HumanLinkBridge:
    ...
    async def _handle_approval_request(self, payload: dict, ws):
        ...
        # Call HumanLink SDK
        success, assertion = await self._authenticate_via_sdk(command, agent_id)
    ...
    async def _authenticate_via_sdk(self, command: str, agent_id: str) -> tuple[bool, Optional[dict]]:
                    ...
                    try:
                        async with session.get(
                            ...
                            if status == "completed":
                                ...
                                return True, status_data.get("assertion")
                            elif status == "failed":
                                logger.info(f"HumanLink auth: denied — {status_data.get('error', '')}")
                                return False, status_data.get("assertion")
